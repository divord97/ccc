...
export default function InternalCallReportPage() {
  ...
  const load = async () => {
    setLoading(true);
    try { const res = await reportApi.internal(params); setData(Array.isArray(res.data) ? res.data : []); } catch { /* */ }
    setLoading(false);
  };
